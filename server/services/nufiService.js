...
const validateCredentials = () => {
  const subscriptionKey = process.env.NUFI_SUBSCRIPTION_KEY;
  ...
  if (apiKey === 'your_api_key_here') {
    throw new Error('Please update NUFI_API_KEY in .env file with your actual key.');
  }

  return { subscriptionKey, apiKey };
};

// Create axios instance with auth
const createNufiClient = () => {
  const { subscriptionKey, apiKey } = validateCredentials();

  const headers = {
    'Content-Type': 'application/json',
    'Accept': 'application/json',
    'Ocp-Apim-Subscription-Key': subscriptionKey || apiKey
  };

  if (apiKey) {
    headers['NUFI-API-KEY'] = apiKey;
  }
...
const renapo = async (params) => {
  const normalized = {
    tipo_busqueda: params.tipo_busqueda || 'datos',
    clave_entidad: params.clave_entidad,
    dia_nacimiento: params.dia_nacimiento,
    mes_nacimiento: params.mes_nacimiento,
    anio_nacimiento: params.anio_nacimiento,
  ...
  const cleanParams = Object.entries(normalized).reduce((acc, [key, value]) => {
  ...
  return await makeApiCall('renapo-curp', '/curp/v1/consulta', cleanParams, 'renapo-curp');
};
